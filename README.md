...
- Implemented: Alpha Vantage adapter, strict OHLCV validation, private disk cache, explicit live/auto/demo modes, deterministic fictional scenarios, Python indicators, bounded OpenAI Responses requests, and the complete three-round browser game.
- Verified locally on Windows with Python 3.14.3: **25 automated tests passed**, Python compilation passed, and a Chrome playthrough covered research, commitments, reveals, final timeout and final score. Browser restart flow is also checked below in the development log. No external calls occur in the test suite.
...
Every validated bar contains `date`, `open`, `high`, `low`, `close`, `volume`. Dataset metadata contains `source`, `symbol`, UTC `downloaded_at`, and `price_basis`. Provider reverse chronology is explicitly normalized into ascending order, then validated. Duplicate JSON keys/dates, invalid date order, non-finite or non-positive prices, invalid volume, inconsistent OHLC, incomplete fields and fewer than 65 sessions are rejected. Three distinct real cutoffs need at least 67 sessions and three usable screened windows. Missing sessions are never filled with invented prices.

| Mode | Behavior |
| --- | --- |
| `demo` | Generates deterministic fictional data without a stock request or stock credential. Persistent **SYNTHETIC DEMO** banner and `FICTION-*` identities. |
...
Successful downloads are atomically cached under ignored `.local/cache/`. All rounds and questions reuse the same loaded data. A game restart reuses the process's loaded dataset; restart the server in live mode to deliberately request fresh data. Auto caches have **no age expiration** because these are historical exercises, not current quotes. Download time is available after reveal. Failed live loads have a 60-second local cooldown. Cache-read errors are surfaced before trying the API; failed cache writes are announced while valid downloaded data remains playable.
...
Real cutoffs are chronological, using the first, middle and last usable windows. With compact history, research windows overlap. Earlier reveals only show sessions that are historical by later cutoffs. The chosen single real ticker becomes known after the first reveal even though subsequent research payloads still omit it. Deterministic demo scenarios repeat on replay; this is a local learning game, not a competitive anti-cheating system.
